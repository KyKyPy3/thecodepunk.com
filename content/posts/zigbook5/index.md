...
Давайте детально рассмотрим все три области памяти, как Zig размещает в них объекты, и как эти области взаимодействуют между собой.
...
Приимущества использования статической области памяти:
...
Все переменные созданные на стеке имеют четкий и определенный срок службы, привязанный к области видимости функции, в которой они были обьявлены. Причем фрейм стека создается не только в рамках области видимости функции, а в рамках любой области видимости, определяемой с помощью блоков кода, таких как if, while, for и т.д.
...
pub fn main() void {
    recursive(10_000); // Это приведет к Stack Overflow
}
```
