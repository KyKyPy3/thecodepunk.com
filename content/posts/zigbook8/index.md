...
Если мы хотим изменять переменную внутри функции, то у нас есть два вариант - вернуть новое значение из функции или передать в функцию указатель. Как мы помним из главыв про указатели, чтобы передать в функцию указатель надо воспользоваться оператором `&`:
...
Этот подход полезен для реализации различных стратегий или создания функций высшего порядка. Например, его можно использовать для каррирования — функциональной техники, при которой функция, принимающая несколько аргументов, преобразуется в последовательность функций, каждая из которых принимает только один аргумент. Каррирование позволяет разбить функцию с несколькими аргументами на цепочку функций, каждая из которых принимает только один аргумент и возвращает новую функцию, ожидающую следующий аргумент. Например, функция `add(a: i32, b: i32) i32` может быть преобразована в `add(a: i32) fn(i32) i32`, где add(5) возвращает функцию, которая принимает b и возвращает 5 + b. Давайте взглянем на код реализующий данную технику:
