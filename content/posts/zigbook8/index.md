...
Тип такого sentinel среза обозначается как `[:sentinel]T`, где `sentinel` — это значение-маркер. Внашем примере маркером конца среза является `0`.
...
Строки созданные с помощью строковых литералов в Zig -  это sentinel строки с маркером `0` и типом данных `[]const u8`. Компилятор автоматически приводит тип нашей строки к типу `[N:0]const u8`. Это сделанно для более удобной совместимости с языком C. Хотя конечно можно создавать строки из массивов символов:
