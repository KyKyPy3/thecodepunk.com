...
Теперь давайте раасмотрим как читать данные из стандартного потока ввода. Для этого по аналогии с `stdout`, нам надо получить аналогичным образом ссылку на `stdin`. Давайте рассмотрим пример:
...
Теперь давайте рассмотрим функцию `std.fmt.allocPrint`. Эта функция полезна когда вы не знаете заранее размер буффера, который бы вместил всю форматированную строку. Давайте взглянем на код:
...
В этом коде мы используем стандартный аллокатор `GeneralPurposeAllocator` и передаём его в функцию `allocPrint`. Она использует аллокатор для выделения памяти под форматированную строку.
