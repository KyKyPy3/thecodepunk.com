...
В этом примере `inline` в блоке else приведет к тому что компилятор развернет все возможныеварианты для всех возможных типов `T`, используемых в программе, и в нашем случае вернет значение 4, что позволяет избежать избыточных проверок во время выполнения.
...
* Блоки кода могут иметь метки и возвращать значения, что делает их похожими на маленькие функции.
* Условные операторы `if` и `switch` также могут возвращать значения, что позволяет писать более компактный и выразительный код.
* Циклы `while` и `for` поддерживают дополнительные конструкции, такие как постусловие или работа с опциональными типами.
* Все конструкции управления потоком могут быть вложенными и поддерживают использование меток для более сложного управления выполнением.
