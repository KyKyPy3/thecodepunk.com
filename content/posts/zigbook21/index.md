...
Несмотря на то что Zig предоставляет отдельный набор типов, совместимых с C, при использовании примитивных типов данных компилятор Zig автоматически приводит их к соответствующим C-типам при необходимости. Это означает, что вам не нужно вручную заботиться о приведении типов при работе с такими значениями, как целые числа, числа с плавающей точкой и логические значения — Zig сам обеспечит корректное соответствие типам C.

Таким образом, при передаче примитивных данных в C-функции всё происходит прозрачно для разработчика — Zig самостоятельно обеспечивает правильное соответствие по размеру, выравниванию и представлению данных. Это значительно упрощает взаимодействие с C-кодом на базовом уровне.

Однако, как только мы выходим за рамки примитивных типов, ситуация становится немного сложнее. Особенно это касается таких типов, как срезы (slices), строки, структуры (structs) и массивы.
...
pub fn main() void {
    const zigString: []const u8 = "Привет, мир!";
    _ = c.printf(@ptrCast(zigString));
}
```

Теперь наш пример успешно компилируется и работает. Однако на самом деле нам даже не нужно было использовать функцию `@ptrCast` — достаточно было передать в функцию `printf` указатель на строку в стиле Zig, используя поле `ptr` у нашей строки:
...
pub fn main() void {
    const zigString: []const u8 = "Привет, мир!";
    _ = c.printf(zigString.ptr);
}
```

При необходимости передачи массива из Zig в C вы можете действовать аналогичным образом.
...
const c = @cImport({
    @cInclude("stdio.h");
    @cInclude("/Users/roman/Projects/zig/simple/src/user.h");
});
...
Если мы запустим этот код то он успешно выполнится и мы увидим:
...
В некоторых из наших примеров уже встречались конструкции `@cImport` и `@cInclude`, но мы ещё не обсуждали, что именно они делают и как работают.
...
Эта команда соберет нашу программу для x86_64-архитектуры Linux. Однако, если вы хотите интегрировать сборку C-кода в полноценную сборочную систему Zig, например, при использовании build.zig, то это можно сделать гораздо удобнее и гибче. Такой подход особенно полезен, когда проект состоит как из Zig, так и из C-файлов, или когда вы хотите использовать заголовочные файлы и библиотеки на C.
...
Теперь если мы выполним команду `zig build run`, то наша программа будет собрана и запущена.
...
Внутри блока `@cImport` можно использовать директиву `@cInclude` для подключения конкретных заголовочных файлов. Это аналогично директиве `#include` в языке C.
...
Есть еще один момент, который стоит упомянуть, прежде чем мы двинемся дальше. В секции `@cImport` мы можем использовать директиву `@cDefine` для того, чтобы переопределять макросы из заголовочных файлов на языке C. Давайте изменим директиву `@cImport` в нашем файле на следубщий и посмотрим как изменится вывод нашей программы:
...
Теперь если мы запустим нашу программу, то увидим следующий вывод:
...
Еще один из вариантов использования C библиотеки в Zig - это использование функций из статической библиотеки. Для того чтобы продемонстрировать как работать со статическими библиотеками нам необходимо изменить два файла - файл сборки и наш файл `main.zig`. давайте начнем с файла сборки:
...
В файл сборки мы внесли следующие изменения. Сначала добавляется сборка нашей статической библиотеки с использованием возможностей Zig по сборке C-кода. Затем, при сборке основного приложения, мы подключаем (линкуем) эту статическую библиотеку с помощью функции `linkLibrary`. И это все измениения, которые нужно сделать в файле `build.zig`. давайте теперь рассмотрим как изменится файл `main.zig`:
...
В результате у нас появится два файла `libziglib.a` и `libziglib.a.o`. Это наша статическая библиотека, которую мы можем использовать в C-коде. Давайте теперь рассмотрим как будет выглядеть на код на C:
