...
Существует приём, позволяющий эффективно хранить большое количество структур. Он не является уникальным для Zig и используется во многих языках программирования — это техника хранения полей структур в виде отдельных массивов (так называемый Structure of Arrays, или SoA).
...
Как мы видим при добавлении элементов в наш массив `users` мы теперь передаем аллокатор. Это необходимо, так как внутренние массивы в которых хранятся поля нашей структуры User динамические и если потребуется увеличить размер одного из этих массивов, то будет использоваться переданный аллокатор. Второй важный момент, это то как мы получаем доступ к нашим элементам внутри нашего типа данных. Для того чтобы получить доступ к значениям конкретного поля нам необходимо вызвать метод `items()` передав в него имя поля, к которому мы хотим получить доступ. Если вам необходимо итерироватся по нескольким полям, то метод `items` лучше вызывать у среза полученного через метод `slice`, а не напрямую у типа `MultiArrayList`.

Второй важный момент, что метод `items()` вернет вам итератор ссылок на элементы массива, что позволяет вам в том числе и изменять элементы в цикле.
