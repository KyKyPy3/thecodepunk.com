...
Если же требуется работать сразу с несколькими полями, рекомендуется сначала вызвать метод `slice()` на экземпляре `MultiArrayList`, а уже затем применять к полученному объекту метод `items()`. Такой подход более эффективен, поскольку позволяет избежать лишнего копирования или пересоздания промежуточных структур.

Однако при этом важно помнить об одной особенности: полученные срезы на значения полей становятся невалидными при любом изменении структуры `MultiArrayList`, которое может привести к перемещению данных в памяти (например, при добавлении новых элементов). Поэтому каждый раз после изменения `MultiArrayList` необходимо повторно запрашивать срезы с помощью `items()` — это обеспечит корректный доступ к актуальным данным.
