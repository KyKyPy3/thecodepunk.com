...
Как возможно вы уже заметили, структура `ArrayList` по сути является просто оберткой над обычным массивом с дополнительным кодом, управляющим контролем за выделением и освобождением памяти. При этом когда мы хотим получить доступ к элементам хранящимся в `ArrayList` мы можем напрямую обращаться к обернотому массиву `items` для итерирования или изменения данных. Единственное важно помнить что получать ссылки на элементы массива `items` опасно, так как в случае перемещения данных все указатели на старый сегмент станут невалидными.

Иногда бывает удобно получить доступ к обернутому массиву `ArrayList` и даже отцепить его от структуры. Например если вы накопили внутри массива нужное количество элементов и хотите передать их дальше, сбросив при этом состояние `ArrayList`. Для этого у типа есть метод `toOwnedSlice`, который вернет срез с данными нашего `ArrayList`, а внутри структуры создаст новый массив. По факту произойдет передача владения данными от нашего массива к вызывающему коду, что потребует от вызывающего когда озаботится освобождением памяти занимаемой нашим срезом.
