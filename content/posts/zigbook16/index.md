...
Как можно заметить, при использовании нашей структуры `User` в качестве элемента массива компилятор установил начальную ёмкость массива равной 5. В случае же с типом данных `u8` компилятор определил начальную ёмкость как 128. Из чего можно сделать вывод что у Вас нет гарантий, что компилятор будет каждый раз выделять одинаковое количество слотов в массиве для разных типов данных. Так как операция изменения емкости массива довольно затратная, то считается хорошим тоном задавать начальное значение емкости массива, чтобы избежать ненужных перемещений.
