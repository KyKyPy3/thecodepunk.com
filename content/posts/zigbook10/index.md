...
pub fn main() void {
    ...
    std.debug.print("Size of Example: {}\n", .{@sizeOf(Example)});

    // Смещения полей
    std.debug.print("Offset of a: {}\n", .{@offsetOf(Example, "a")});
    std.debug.print("Offset of b: {}\n", .{@offsetOf(Example, "b")});
    std.debug.print("Offset of c: {}\n", .{@offsetOf(Example, "c")});
...
Offset of b: 0
Offset of c: 5
Address of a: u8@10026508c
Address of b: u32@100265088
Address of c: u8@10026508d
```
...
### Extern структуры
Язык Zig ориентирован на бесшовную интеграцию с существующим C-кодом, и одним из ключевых инструментов для этого являются extern-структуры. Extern-структура — это специальный тип структуры, который гарантирует совместимость своего представления в памяти с правилами размещения и выравнивания языка C.
