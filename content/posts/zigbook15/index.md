...
В этом примере мы используем хеш-таблицу, где в качестве ключа мы используем строковые литералы и у нас нет ошибки компиляции, так как наш тип хеш-таблицы `StringHashMap` знает как хешировать и сравнивать строки.
...
Затем мы создаем `HashMap` с типом ключа `User`, типом значения `i32`, контекстом хеширования `User.HashContext` и стандартным максимальным процентом загрузки. Теперь мы можем использовать нашу структуру `User` в качестве ключа в хеш-таблице, даже несмотря на то, что она содержит срез.

Этот подход дает нам полный контроль над тем, как хешируются и сравниваются наши пользовательские типы, что особенно полезно для сложных структур данных или когда мы хотим оптимизировать производительность хеш-таблицы для конкретного случая использования.
...
Как мы видим наш план не удался. Значение пользователя хранимое в хеш-таблице не изменилось как мы планировали. Все дело в том, что метод `get` возвращает копию значения, а не ссылку на него. Поэтому, когда мы изменяем значение `user.name`, мы изменяем только копию, а не оригинальное значение в хеш-таблице. Для того чтобы изменить значение в хеш-таблице, нам нужно получить ссылку на значение и для этого у таблицы есть метод `getPtr` который возвращает указатель на значение:
...
В языке Zig нет отдельного типа данных для множеств, но эту задачу можно решить с помощью хеш-таблицы, в которой значения имеют тип `void`. В этом случае мы не расходуем память на хранение, например, булевого признака, и при этом можем легко итерироваться по множеству, используя итератор по ключам хеш-таблицы.
...
### ArrayHashMap
ArrayHashMap - это гибридная структура данных, сочетающая преимущества динамического массива и хеш-таблицы. В Zig она реализована в стандартной библиотеке как `std.ArrayHashMap` и `std.AutoArrayHashMap` (с автоматическим определением типов). Эта структура особенно полезна, когда вам нужны быстрые lookup-операции по ключу, но также важна сохранение порядка вставки элементов. Этот тип хеш-таблицы хранит ключи и значения в массивах, что позволяет легко итерироваться по элементам в порядке их добавления. Если заглянуть в код данного типа данных, то можно увидеть что внутри используется уже известная нам структура `MultiArrayList`. Давайте рассмотрим пример использования `std.AutoArrayHashMap`:
...
Данный тип хеш-таблицы почти ничем не отличается от обычной, кроме операций удаления элементов. Так как внутри данной структуры используется `MultiArrayList`, то операции удаления элементов выполняются также как и в типе данных `MultiArrayList`. Если вам необходимо удалить элементы без сохранения порядка, то используйте метод `swapRemove`, если же вам важен порядок элементов в хеш-таблице, то используйте метод `orderedRemove`. В целом есть ряд задач, для которых полезно наличие такой хеш-таблицы, гарантирующей сохранение порядка элементов, но надо всегда помнить, что операции удаления элементов в хеш-таблице с сохранением порядка имеют сложность O(n), а не O(1), как в обычной хеш-таблице.

### StringArrayHashMap
Аналогично тому, как `StringHashMap` позволяет использовать строки в качестве ключей в стандартной хеш-таблице, мы можем создать тип `StringArrayHashMap`. Он работает так же, как и `ArrayHashMap`, с той лишь разницей, что умеет корректно обрабатывать строковые ключи.

Рассмотрим простой пример использования `StringArrayHashMap`:
...
pub fn main() !void {
    ...
    var map = std.StringArrayHashMap(User).init(allocator);
    defer map.deinit();
...
## Заключение
В данной главе мы рассмотрели хеш-таблицу (HashMap) — одну из наиболее важных и часто используемых структур данных в программировании. Хеш-таблицы обеспечивают эффективный доступ к данным по ключу за константное время O(1) в среднем случае, что делает их незаменимыми для многих практических задач.

Мы познакомились с несколькими реализациями хеш-таблиц в стандартной библиотеке Zig:

- **AutoHashMap** — универсальная реализация, автоматически генерирующая функции хеширования для примитивных типов и простых структур.
- **StringHashMap** — специализированная реализация для строковых ключей, корректно обрабатывающая сравнение строк по содержимому.
- **HashMap** — базовая реализация, позволяющая определить собственные функции хеширования и сравнения для любых типов данных.
- **ArrayHashMap** и **StringArrayHashMap** — реализации, сохраняющие порядок вставки элементов, что может быть полезно в определенных сценариях.

Мы также изучили важный аспект архитектуры стандартной библиотеки Zig — **Unmanaged** типы, которые не хранят аллокатор и требуют его явной передачи в каждый метод. Такой подход обеспечивает большую гибкость и контроль над управлением памятью.

Умение эффективно использовать хеш-таблицы — важный навык для любого программиста, и Zig предоставляет все необходимые инструменты для работы с этой структурой данных, сохраняя при этом производительность и контроль над памятью, которые являются ключевыми принципами языка.
