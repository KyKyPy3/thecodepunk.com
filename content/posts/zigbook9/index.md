...
По умолчанию, все методы структуры в Zig являются приватными - они доступны только внутри модуля, где определена структура. Если вы хотите сделать метод доступным извне, вы должны пометить его ключевым словом `pub`:
