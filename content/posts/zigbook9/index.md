...
### Видимость структур
По умолчанию все структуры, объявленные в модуле, не видны за его пределами. Чтобы сделать структуру доступной в другом модуле, необходимо пометить её ключевым словом pub.

В Zig все поля структуры по умолчанию являются публичными, то есть доступны за пределами самой структуры. Это отличается от Rust, где поля структуры изначально закрыты для внешнего доступа. В Zig используется принцип открытости, что упрощает работу с данными, но усложняет реализацию инкапсуляции в рамках объектно-ориентированного подхода.
...
Более подробно работу с анонимными структурами мы рассмотрим в следующей главе, когда будем разговаривать про кортежи.
