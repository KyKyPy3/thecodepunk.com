...
Давайте подробнее рассмотрим, как это работает:
	1.	Функция принимает параметры сборки, такие как оптимизация, целевая платформа и т.д.
	2.	С помощью zigTriple из целевой платформы формируется строка вида x86_64-linux-gnu, armv7-windows-gnu и т.п.
	3.	Эта строка используется для создания отдельной папки внутри выходного каталога, чтобы артефакты не смешивались между собой при сборке под разные платформы.
	4.	Затем внутри функции последовательно создаются сборочные шаги для библиотеки и для исполняемого файла.
