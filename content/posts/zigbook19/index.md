...
Итак мы научились собирать нашу библиотеку и приложение. Но что нам делать если например наше библиотека требует от нас наличие другой библиотеки, которая например устанавливается в систему напрямую. В этом случае у сборочной системы есть два параметра, позволяющие управлять доступом к системным библиотекам - `linkLibC`, `linkLibCpp` и `linkSystemLibrary`. Везов методов `linkLibC` или `linkLibCpp` добавит к вашему модулю стандартную библиотеку C или C++, а `linkSystemLibrary` добавит к вашему модулю системную библиотеку, которая будет искаться в системных путях. Давайте взглянем на пример использования `linkSystemLibrary`:
...
pub fn build(b: *std.Build) void {
    ...
    // Если нужны дополнительные флаги линковки (зависит от системы)
...
Для того чтобы добавить пользовательский параметр сборки необходимо использовать метод `option` у системы сборки и передать ему имя параметра сборки, тип параметра и описание, что делает данный параметр для вывода пользователю в команде помощи. Давайте реализуем следующую программу, которая в зависимости от параметра переданного во время сборки будет определать использовать ей собственную реализацию функции или использовать реализацию из статически слинкованной библиотеки. Это конечно искусственный прием, но он поможет нам разобрать ряд интересных моментов использования пользовательских параметров при сборке. давайте начнем с нашего сборочного скрипта:
