...
Для того, чтобы при использовании диапазонов получить конкретное число, можно использовать оператор `|n|`. В данном примере, если наше число будет в интервале от 4 до 6, то мы получим конкретное число, которое будет присвоено переменной `n`. Это позволяет использовать фактическое значение внутри блока кода.
